...
const Starfield: React.FC<StarfieldProps> = ({
  ...
  const resize = () => {
    ...
    if (sd.current.cw !== sd.current.w || sd.current.ch !== sd.current.h) {
      ...
      if (!sd.current.star.arr.length) {
        ...
        sd.current.star.arr = sd.current.star.arr.map((star: any[], i: number) => {
          const newStar = [...star];
  ...
  const update = () => {
    ...
    if (sd.current.star.arr.length > 0) {
      sd.current.star.arr = sd.current.star.arr.map((star: any[]) => {
        ...
        if (newStar[0] > sd.current.x << 1) { newStar[0] -= sd.current.w << 1; newStar[7] = false; }
        if (newStar[0] < -sd.current.x << 1) { newStar[0] += sd.current.w << 1; newStar[7] = false; }
        newStar[1] += mouse.current.y >> 4;
        if (newStar[1] > sd.current.y << 1) { newStar[1] -= sd.current.h << 1; newStar[7] = false; }
        if (newStar[1] < -sd.current.y << 1) { newStar[1] += sd.current.h << 1; newStar[7] = false; }
        newStar[2] -= compSpeed;
        if (newStar[2] > sd.current.z) { newStar[2] -= sd.current.z; newStar[7] = false; }
        if (newStar[2] < 0) { newStar[2] += sd.current.z; newStar[7] = false; }
        newStar[3] = sd.current.x + (newStar[0] / newStar[2]) * ratio;
        newStar[4] = sd.current.y + (newStar[1] / newStar[2]) * ratio;
        return newStar;
      });
  ...
  const draw = () => {
    ...
    sd.current.star.arr.forEach((star: any[]) => {
